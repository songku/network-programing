 ...
 * @LastEditTime: 2021-11-30 23:07:30
 */
#include <iostream>
#include <winsock2.h>
...
DWORD WINAPI AsyncServerEcho(const LPVOID arg)
{
    ...
    do
    {
        ...
        res = recvvl(temp->clientSocket, recv_data,BUFFSIZE);
        if (res > 0)
        {
            cout << recv_data << endl;
            ...
            res = send(temp->clientSocket, (char*)&slength,sizeof(unsigned int), 0);
            if (res == SOCKET_ERROR)
            {
            ...
            //发送实际数据
            res = send(temp->clientSocket, echo_buff, int(strlen(echo_buff)), 0);
            if (res == SOCKET_ERROR)
            {
                cout << WSAGetLastError() << "发送错误!" << endl;
                res = -1;
