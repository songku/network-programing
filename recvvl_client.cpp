 ...
 * @Description: 
 * @Author: dive668
 * @Date: 2021-11-30 20:52:46
 * @LastEditTime: 2021-12-03 22:46:06
 */
#include <iostream>
#include <time.h>
#include <winsock2.h>
...
int main(int argc, char** argv)
{
    ...
    {
        if (*send_data == 'Q' || *send_data == 'q') //退出
        {
            cout << "输入结束!" << endl;
            // 关闭发送
            res = shutdown(ConnectSocket, SD_SEND); //关闭套接字通道
            if (res == SOCKET_ERROR)
            {
        ...
        // 发送长度数据
        length = (unsigned int)strlen(send_data); //注意这里强制类型转换
        ...
        // 接收数据 -- 阻塞模式接收
        int rec_val;
        rec_val=recvvl(ConnectSocket,recv_data,RecvBuffer); //接收服务器回送数据
