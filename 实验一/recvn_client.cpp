 ...
 * @LastEditTime: 2021-12-03 21:35:47
 */
#include <iostream>
#include <time.h>
#include <winsock2.h>
...
int recvn(SOCKET s,char *recvbuf,unsigned int fixedlen)
{
    ...
    while(cnt>0)
    {
        iResult=recv(s,recvbuf,cnt,0); //保存返回值
        if(iResult<0)
        {
            //数据接收出现错误，返回失败
...
int main(int argc, char** argv)
{
    ...
    memset(send_data, 0, SendBuffer); //发送缓冲区初始化为0
    memset(recv_data, 0, RecvBuffer); //接收缓冲区初始化为0
    cout<<"输入五个字节的数据(注:包含回车，例如:1234):"; //提示信息
    while (fgets (send_data, SendBuffer, stdin)!=NULL) //获取输入
    {
        if (*send_data == 'Q' || *send_data == 'q') //用户终止
        {
            cout << "输入结束!" << endl;
            // 关闭发送
        ...
        if(strlen(send_data)!=5) //接收的数据长度不等于5
        {
            cout<<"输入字节数据有误，请重新输入:";
            continue;
        }
