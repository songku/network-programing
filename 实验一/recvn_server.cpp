...
int main(int argc, char** argv)
{
	...
	for (; ; )
	{
        ...
        do
        {
            memset(recv_data, 0, BUFFSIZE); // 清空接收缓存
            res = recvn(ClientSocket, recv_data,fixed_length); // 接收数据
            if (res > 0) //返回值大于0，接收成功
            {
                cout << recv_data << endl;
                // 回射接收的数据
                ...
                res = send(ClientSocket, echo_buff, int(strlen(echo_buff)), 0); //回射
                if (res == SOCKET_ERROR)
                {
