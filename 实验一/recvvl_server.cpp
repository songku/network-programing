 ...
 * @Description: 
 * @Author: dive668
 * @Date: 2021-11-30 16:28:07
 * @LastEditTime: 2021-12-04 09:37:07
 */
#include <iostream>
#include <winsock2.h>
...
int main(int argc, char** argv)
{
	...
	for (; ; )
	{
        ...
        do
        {
            ...
            // 接收数据
            res = recvvl(ClientSocket, recv_data,BUFFSIZE); //接收数据
            if (res > 0)
            {
                cout << recv_data << endl;
                // 回射接收的数据
                ...
                res = send(ClientSocket, (char*)&slength,sizeof(unsigned int), 0); //send()长度数据
                if (res == SOCKET_ERROR)
                {
                ...
                //发送实际数据
                res = send(ClientSocket, echo_buff, int(strlen(echo_buff)), 0); //send()实际数据
                if (res == SOCKET_ERROR)
                {
                    cout << WSAGetLastError() << "发送错误!" << endl;
                    res = -1;
